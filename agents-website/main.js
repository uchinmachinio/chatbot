...
const windowSec = 60, tickSec = 10, maxPoints = 2000;
...
function initHRChart() {
    const layout = {
        ...
        yaxis: { title: "BPM", range: [lastBpm - 10, lastBpm + 10], gridcolor: "rgba(255,255,255,0.06)", tickfont: { color: "#9ca3af" }, titlefont: { color: "#9ca3af" } },
        font: { color: "#e5e7eb" }, uirevision: "stream", showlegend: false
    };
    const trace = {
        x: [], y: [],
        mode: "lines+markers",
        line: { width: 2, color: "#38bdf8", shape: "spline", smoothing: 0.6 },
...
function pushHR(ts, bpm) { q.push([ts, bpm]); if (!raf) raf = requestAnimationFrame(flush); }
